...
24. 默认不显示文件选择框；点击“多选”后才显示本页全选和批量操作条，退出后恢复普通文件列表。
25. 磁链开关默认关闭且只有已登录管理员可修改；开启后合法 BTIH 磁链可提交，缺少或非法哈希会被拒绝。
26. 多选可跨分页保留且最多选择 50 个；摘要同时显示数量与文件总大小，批量下载不触发一次性下载，批量删除需要确认和管理密码。
27. “打包 ZIP”下载包含全部所选文件，完整传输后为每个源文件增加一次下载计数，并且 `data/.batch-archive-*.zip` 不残留。
28. 批量续期无需密码，所有选中文件保持原有保留期限并从同一时间点重新计算过期时间。
29. 文件数量不足一页或切换筛选/多选模式时，文件分页器始终固定在文件卡片底部。
