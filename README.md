...
一个轻量的临时下载站：管理员粘贴 HTTP/HTTPS 下载链接，或按需在后台开启磁链，后端通过 aria2 下载到 `downloads/`，朋友打开页面下载文件。项目只使用 Python 3 标准库和 `aria2c`，不需要数据库，不使用 React/Vue/Vite/Next.js。
...
- 文件列表默认隐藏选择框，点击标题栏“多选”后进入批量模式；选择可跨分页保留，实时显示文件数量与总大小，并支持普通下载、临时 ZIP 打包、无需密码的批量续期和带密码确认的批量删除。
- 批量 ZIP 在 `data/` 中流式生成并在传输完成或中断后删除，不加入可用文件列表；同一时间只运行一个打包任务。
...
- 管理后台提供默认关闭的磁链开关；开启后只接受包含 40 位十六进制或 32 位 Base32 BTIH 哈希的磁链。
...
├── data/
│   ├── .gitkeep
│   ├── settings.json       # 运行时站点设置（默认不生成，首次保存后台设置后创建）
...
3. 输入 HTTP/HTTPS 链接，或点击粘贴按钮读取剪贴板；批量添加时每行填写一个链接。磁链需先在管理后台开启。
4. 单个 HTTP/HTTPS 链接可以设置自定义文件名；批量链接和磁链使用 aria2 解析的默认文件名。
5. 提交后首页会显示添加结果，不会跳转到单独的成功页面。
6. 在文件工作区预览或下载文件。点击“多选”后可跨分页选择文件，再执行普通下载、打包 ZIP、续期或批量删除；浏览器可能首次询问是否允许多个文件下载。

磁链开关只决定是否允许把合法 `magnet:?` 链接交给 aria2。Tracker、DHT、UDP 和入站端口仍由部署机器负责；单文件磁链下载完成后会进入现有文件列表，多文件种子可能由 aria2 建立子目录，而当前轻量文件列表不展开目录。
...
文件的首次入库时间、保留期限、`download_count` 和 `preview_count` 保存在 `data/filemeta.json`。普通下载和批量 ZIP 只有在响应体完整传输后才增加下载计数；HEAD、在线预览、Range 请求和中断传输不会增加下载计数。图片和视频每次打开预览页增加一次预览计数，播放器后续的 Range 请求不重复增加。一次性下载完整传输后会先计数再删除文件，因此对应元数据也随文件一起清理。
...
- Backup：只备份 `filemeta.json`、`task_retention.json` 和 `settings.json`，不复制下载文件、密码或 aria2 secret，最多保留 10 份。
...
MAX_BATCH_ARCHIVE_BYTES=4294967296
ARIA2_RPC_TIMEOUT=3
...
- `MAX_BATCH_ARCHIVE_BYTES` 限制一次 ZIP 打包的源文件总大小，默认与单文件上限相同；打包还会额外保留 64MiB 磁盘余量。
